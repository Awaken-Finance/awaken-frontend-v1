...
export function isWebview() {
  return !!navigator.userAgent.match(webviewRegExp) || 'ReactNativeWebView' in window;
}
...
export function isActivityBrowser() {
  return isReactNative() && !isPortkeyAppWithDiscover();
}

export function isNightElfApp() {
  return isWebview() && !isReactNative() && 'android' in window;
}
