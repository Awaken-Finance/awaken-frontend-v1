...
function convertCharRangeToRegExp(range) {
  const reStr = range
    .map((range) => {
      if (range[0] === range[1]) {
...
const isValidString = function (str, options) {
  const cnRe = options.includePunctuation ? reWithPunctuation : reWithoutPunctuation;
